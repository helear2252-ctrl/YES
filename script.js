document.addEventListener('DOMContentLoaded', () => {
    ...
    /* --- Site Content AI Assistant --- */
    const aiAssistant = document.getElementById('siteAiAssistant');
    ...
    const siteKnowledgeBase = [
        {
            keywords: ['網站', 'portfolio', '作品集', 'machine learning', 'ml', '這個網站', '是什麼'],
            answer: '這個網站是一個 Machine Learning Portfolio，展示機器學習演算法、互動式線性迴歸展示、Outlier Detection、CRISP-DM 流程與 Streamlit 後端整合概念。'
        },
        {
            keywords: ['有哪些', '演算法', 'algorithm', 'algorithms', '十大', '機器學習'],
            answer: '本網站介紹的機器學習演算法包含 Linear Regression、Logistic Regression、Decision Tree、Random Forest、KNN、SVM、Naive Bayes、K-Means 等主題，並以按鈕切換方式呈現核心概念、應用情境與評估重點。'
        },
        {
            keywords: ['linear regression', '線性迴歸', '迴歸線'],
            answer: 'Linear Regression（線性迴歸）用一條直線描述輸入特徵與連續目標值之間的關係，常用於預測數值，例如房價、銷售量或趨勢。網站中的互動展示也用線性迴歸呈現斜率、截距、雜訊與離群值對模型的影響。'
        },
        {
            keywords: ['logistic regression', '邏輯斯', '邏輯迴歸', '分類'],
            answer: 'Logistic Regression（邏輯迴歸）主要用於分類問題，透過機率估計判斷資料屬於哪一類，常見於二元分類，例如是否流失、是否違約或是否為特定類別。'
        },
        {
            keywords: ['decision tree', '決策樹'],
            answer: 'Decision Tree（決策樹）用樹狀規則切分資料，透過一連串條件判斷完成分類或預測。它的優點是容易解釋，適合用來理解特徵如何影響模型決策。'
        },
        {
            keywords: ['random forest', '隨機森林'],
            answer: 'Random Forest（隨機森林）由多棵決策樹組成，透過集成學習降低單一決策樹過度擬合的風險，通常能提供更穩定的分類或迴歸表現。'
        },
        {
            keywords: ['knn', 'k-nearest', 'nearest neighbor', '最近鄰', 'k 近鄰'],
            answer: 'KNN（K-Nearest Neighbors）會根據資料點之間的距離，參考最接近的 K 個鄰居來進行分類或預測。它直覺易懂，但對特徵尺度與資料量較敏感。'
        },
        {
            keywords: ['svm', 'support vector', '支持向量'],
            answer: 'SVM（Support Vector Machine）會尋找能最大化類別間隔的決策邊界，適合處理分類問題，也能透過 kernel 方法處理非線性資料。'
        },
        {
            keywords: ['naive bayes', 'bayes', '貝氏', '貝葉斯'],
            answer: 'Naive Bayes（樸素貝氏）以機率與貝氏定理為基礎，假設特徵之間相互獨立。它常用於文字分類、垃圾郵件辨識與快速基準模型。'
        },
        {
            keywords: ['k-means', 'k means', 'kmeans', '分群', '聚類'],
            answer: 'K-Means 是一種非監督式分群演算法，會把資料分成 K 個群集，讓同一群內的資料彼此更相似，常用於客群分群、資料探索與模式辨識。'
        },
        {
            keywords: ['outlier', 'outliers', 'outlier detection', '離群', '異常值', 'anomaly'],
            answer: 'Outlier Detection 是偵測資料中明顯偏離一般模式的觀測值。網站中的 Outliers 區塊展示 Top 10 Outlier Observations，用來標記資料集中偏離常態的極端值，協助理解資料品質與模型受影響的程度。'
        },
        {
            keywords: ['crisp', 'crisp-dm', '資料探勘', '流程', 'methodology'],
            answer: 'CRISP-DM 是跨行業資料探勘標準流程，包含 Business Understanding、Data Understanding、Data Preparation、Modeling、Evaluation 與 Deployment。網站用它說明從問題理解、資料處理、建模評估到部署的完整機器學習專案流程。'
        },
        {
            keywords: ['streamlit', '後端', 'backend', 'python', 'api'],
            answer: 'Streamlit 後端在本網站中代表未來可串接的 Python 機器學習運算層，用來處理模型計算、資料分析與互動式結果呈現。目前 GitHub Pages 版本保持純前端展示，不放 API Key，也不直接串接外部 AI API。'
        }
    ];

    const outOfScopeAnswer = '這個問題目前超出本網站知識範圍，你可以詢問機器學習演算法、CRISP-DM、Outlier 或 Streamlit 相關內容。';
    ...
    function getSiteAssistantAnswer(question) {
        const normalizedQuestion = normalizeQuestion(question);
        if (!normalizedQuestion) return '';

        const matchedEntry = siteKnowledgeBase.find(entry =>
            entry.keywords.some(keyword => normalizedQuestion.includes(keyword.toLowerCase()))
        );

        return matchedEntry ? matchedEntry.answer : outOfScopeAnswer;
    }
    ...
    function setAiAssistantOpen(isOpen) {
        if (!aiAssistant || !aiAssistantToggle || !aiChatWindow) return;
        ...
        aiChatWindow.setAttribute('aria-hidden', String(!isOpen));

        if (isOpen && aiChatInput) {
            setTimeout(() => aiChatInput.focus(), 120);
        }
    }

    if (aiAssistant && aiAssistantToggle && aiChatWindow && aiChatForm && aiChatInput && aiChatMessages) {
        aiAssistantToggle.addEventListener('click', () => {
            setAiAssistantOpen(!aiAssistant.classList.contains('is-open'));
        });
        ...
        aiChatForm.addEventListener('submit', (event) => {
            ...
            setTimeout(() => {
                appendAiMessage(getSiteAssistantAnswer(question), 'bot');
            }, 160);
        });
